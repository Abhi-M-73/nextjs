...
const USER_SITE_URL = import.meta.env.VITE_USER_SITE_URL;
...
const AllUsers = () => {
  ...
  const USER_SITE_URL = import.meta.env.VITE_USER_SITE_URL;
  console.log("USER_SITE_URL:", USER_SITE_URL); // Debugging line
  ...
  const handleLoginAsUser = async (user) => {
    try {
      ...
      if (response?.success && response?.data?.token) {
        ...
        // Admin ka current session backup — wapas aane ke liye
        const adminToken = localStorage.getItem("token");
        const adminUser = localStorage.getItem("user");
        sessionStorage.setItem("adminBackupToken", adminToken || "");
        sessionStorage.setItem("adminBackupUser", adminUser || "");

        // Ab user ka session set karo
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={loginAsId === row._id}
            title="Login as this user"
            className="inline-flex items-center gap-1 rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loginAsId === row._id ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
